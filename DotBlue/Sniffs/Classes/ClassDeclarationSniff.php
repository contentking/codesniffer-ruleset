...
class ClassDeclarationSniff implements PHP_CodeSniffer\Sniffs\Sniff
{
	...
	public function process(PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
	{
		...
		if ($tokens[$token['scope_opener']]['line'] === $tokens[$token['scope_closer']]['line']) {
			if ($tokens[$token['scope_opener']]['line'] !== $tokens[$stackPtr]['line']) {
 				$phpcsFile->addError('Both opening and closing brace must be on same line as declaration in case of empty body.', $token['scope_opener'], 'code');
			}
	...
	private function processOpen(PHP_CodeSniffer\Files\File $phpcsFile, $opener)
	{
		$tokens = $phpcsFile->getTokens();
		...
		if ($diff !== 2) {
			$fix = $phpcsFile->addFixableError('There must be one empty line before the class body. Found ' . ($diff - 1), $opener, 'code');
	...
	private function processClose(PHP_CodeSniffer\Files\File $phpcsFile, $closer)
	{
		$tokens = $phpcsFile->getTokens();
		...
		if ($diff !== 2) {
			$fix = $phpcsFile->addFixableError('There must be one empty line after the body. Found ' . ($diff - 1), $closer, 'code');
